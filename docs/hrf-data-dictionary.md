...
| `LastMatch_PositionCode` | Hipótesis: código de posición jugada (*Role/RoleID*) | 🔵 — la documentación de desarrolladores CHPP confirma el concepto general: el campo `RoleID` en la API de alineaciones de partido *"indicates which formal 'slot' (Role) the player has filled in the match"* — esto confirma que Hattrick sí tiene un concepto oficial de "código numérico de posición jugada". Pero no se encontró la tabla oficial que traduzca cada número (100, 101, 105...) a un nombre de posición, así que el campo del `.hrf` en sí queda sin la traducción confirmada |
| `LastMatch_Type` | Código de tipo de partido | ❓ — sin tabla oficial encontrada |
