...
- `LastMatch_PositionCode` — **investigación cerrada, significado desconocido** (ver `hrf-data-dictionary.md`): Hattrick documenta oficialmente `RoleID` (1–21) y `PositionCode` (1–11) en CHPP, pero ninguna escala coincide con los valores observados en el `.hrf` (100–113), y el grupo `LastMatch_*` no tiene especificación oficial propia. No usable para decisiones tácticas hasta que aparezca evidencia adicional.
...
3. ~~¿Qué significa `LastMatch_PositionCode`?~~ **Resuelta (investigación cerrada): desconocido.** Ver `hrf-data-dictionary.md` — ninguna de las dos escalas oficiales de CHPP (`RoleID` 1–21, `PositionCode` 1–11) coincide con los valores observados. `LastMatch_Type` queda con el mismo patrón (hallazgo colateral, sin cerrar todavía): tampoco coincide con el `MatchType` oficial de CHPP (1–12).
